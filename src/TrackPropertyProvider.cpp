		...
		void enumerate_properties_v5(metadb_handle_list_cref handles, track_property_provider_v5_info_source& source, track_property_callback_v2& callback, abort_callback&) final
		{
			if (callback.is_group_wanted(Component::name.data()))
			{
				...
				if (count == 1)
				{
					...
					if (f.first_played > 0) callback.set_property(Component::name.data(), 3.0, "First Played", PlaybackStatistics::timestamp_to_string(f.first_played));
					if (f.last_played > 0) callback.set_property(Component::name.data(), 4.0, "Last Played", PlaybackStatistics::timestamp_to_string(f.last_played));
					if (f.added > 0) callback.set_property(Component::name.data(), 5.0, "Added", PlaybackStatistics::timestamp_to_string(f.added));
				}
				else
				{
